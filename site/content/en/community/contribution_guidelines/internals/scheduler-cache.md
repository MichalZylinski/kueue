...
Package: `pkg/cache/scheduler/`. The mirror image of Chapter 2: everything that *is* admitted, and how much quota it consumes, aggregated over the cohort hierarchy.

## 3.1 The live cache

`Cache` (`cache.go`) maintains, under one lock:

- `clusterQueues` — internal `clusterQueue` structs (`clusterqueue.go`) with per-flavor-resource usage, admitted workload map, flavor/check readiness (a CQ whose ResourceFlavor or AdmissionCheck doesn't exist is *inactive* and the reason is reported in the CQ status);
- the cohort hierarchy (via `pkg/cache/hierarchy/`, generic parent/child plumbing reused by the queue manager too);
- the TAS cache (`tas_cache.go`, per-flavor node capacity — Chapter 7);
...
Quota math is unified for ClusterQueues and Cohorts in `resource_node.go`:
...
`Snapshot(ctx)` (`snapshot.go`) deep-copies the whole structure into `ClusterQueueSnapshot`/`CohortSnapshot` (`clusterqueue_snapshot.go`, `cohort_snapshot.go`). The scheduling cycle then works **only on the snapshot**:
...
Admission is optimistic (scheduler.go:850–930):
...
Per-CQ/LQ/cohort gauges (`clusterqueue_metrics.go`, `localqueue_metrics.go`, `cohort_metrics.go`) are resynced from cache state (`Resync*GaugeMetrics`). Metric bugs ("usage gauge wrong after CQ update") are usually a missing resync call after a mutation.

**Where you'll engage:** quota semantics (lending limits, borrowing), cohort hierarchy features, snapshot performance (a hot area — snapshots are O(cluster state) every cycle), metrics. Tests: `snapshot_test.go`, `cache_test.go`, and scheduler integration suites.
