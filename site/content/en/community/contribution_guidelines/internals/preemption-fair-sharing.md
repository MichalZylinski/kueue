...
Packages: `pkg/scheduler/preemption/` (+ `fairsharing/` inside it), share math in `pkg/cache/scheduler/fair_sharing.go`. Entered whenever the flavor assigner returns `Preempt` mode.
...
- **DRS — dominant resource share** (`fair_sharing.go:41`): for each CQ/cohort node, `max over resources (usage above nominal ÷ lendable capacity in the cohort)`, divided by the node's `.spec.fairSharing.weight`. Zero weight ⇒ infinite share (always the first preemption victim). The admission iterator (`fair_sharing_iterator.go`) admits the entry whose CQ has the *lowest* share, recomputing as usage is assumed.
...
- **Admission Fair Sharing** (KEP-4136, gate `AdmissionFairSharing`) adds *historical usage* penalties per LocalQueue (tracked in `pkg/cache/queue/afs/`, decayed over time) so a queue that ran a lot recently sorts later even if it's not borrowing *now*.
...
Reuse of the eviction machinery ≠ preemption: workload-slice replacement for elastic jobs (`replaceOldWorkloadSlice`, scheduler.go:557), TAS node-failure migration (`issueMigration`, scheduler.go:509). They carry their own reasons — keep it that way (terminology review rule).

**Where you'll engage:** roadmap items "preemption cost" (KEP-7990) and MultiKueue orchestrated preemption (KEP-8303) land here; fair-sharing strategy tuning; priority boost (KEP with `PriorityBoost` gate). This package has the highest test density in the repo (`preemption_test.go`, `scheduler_fs_test.go`, `scheduler_afs_test.go`) — every behavioral change needs table cases across all three policy dials.
