...
- **`Topology` CRD** (`apis/kueue/v1beta2/topology_types.go`): an ordered list of `levels`, each a node label key, from widest to narrowest, e.g. `cloud.provider.com/block`, `.../rack`, `kubernetes.io/hostname`.
...
The TAS cache (`pkg/cache/scheduler/tas_cache.go`, `tas_flavor.go`, `tas_nodes_cache.go`) watches Nodes and non-TAS pods (`tas_non_tas_pod_cache.go` — usage by pods Kueue doesn't manage still consumes node capacity!) and builds, per TAS flavor, a tree of topology domains with free capacity per domain.

At snapshot time (`tas_flavor_snapshot.go`), the flavor assigner asks: *can this pod set's count × requests fit into the domain tree under the requested level?* The algorithm greedily searches the fewest/lowest domains that fit (balanced placement variant behind `TASBalancedPlacement`, `tas_balanced_placement.go`). If yes, the chosen domains are recorded in the assignment; capacity is consumed in the snapshot so co-scheduled workloads in the same cycle don't collide.
