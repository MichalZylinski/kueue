...
Package: `pkg/scheduler/flavorassigner/`. Given one workload and one ClusterQueue snapshot, produce an `Assignment`: for every pod set and every resource, *which flavor*, and *in which mode*.
...
3. Feature adjuncts — TAS feasibility (Chapter 7), DRA device quota (`pkg/dra/`), reclaimable pods (already-finished pods reduce the request — KEP-78).
...
Comparing candidate assignments across flavors happens in the internal `preemptionMode` ordering (flavorassigner.go:473–546). This is a subtle, table-test-heavy area: any change here needs cases in `flavorassigner_test.go` covering all policy combinations.
...
- `updateAssignmentIfNeeded` (scheduler.go:657) revalidates a nominated entry's assignment if the snapshot changed mid-cycle (earlier entries in the same cycle may have consumed quota).
